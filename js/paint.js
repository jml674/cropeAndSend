...
var tool = {
	...
	eraser: function() {
		...
		*/
		...
		c.shadowColor="black";
		//c.globalAlpha = 0.5;
		c.strokeStyle = "#444"; //c.fillStyle; //'#FFF'; //selCol('#FFF');
	},
		...
		this.up = function(e) {
			if(this.status == 4) { //finished selecting
				...
				if(this.dimension.x == 0 && this.dimension.y == 0) { //nothing selected, abort
					...
					if (clickedTool.attr("src").indexOf("crop") != -1) {
						canvas.width = this.dimension.x;
						canvas.height = this.dimension.y;
						c.drawImage(canvassel, 0, 0);
						//console.log(canvas.toDataURL());
						openEditor(canvas.toDataURL(), true);
					}
				}
				iface.txy.innerHTML = '&nbsp;';
